...
public class PotionEffectFlag extends Flag<PotionEffect> {
    ...
    // Also we add extra 19 ticks (almost a second) to avoid the timer constantly going from 15s to 14s and back (Its
    // annoying)
    ...
    public PotionEffect parseInput(FlagContext context) throws InvalidFlagFormat {
        ...
        if (split.length < 1 || split.length > 3) {
            throw new InvalidFlagFormat(
                    "Please use the following format: <effect name> [effect amplifier] [show particles]");
        }

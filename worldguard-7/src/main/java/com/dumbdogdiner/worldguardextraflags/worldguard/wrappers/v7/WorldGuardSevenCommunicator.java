...
public class WorldGuardSevenCommunicator implements WorldGuardCommunicator {
    ...
    static {
        try {
            WorldGuardSevenCommunicator.supportsForceLoad =
                    Chunk.class.getMethod("setForceLoaded", boolean.class) != null;
        } catch (Throwable e) {
    ...
    public boolean doUnloadChunkFlagCheck(org.bukkit.World world, Chunk chunk) {
        for (ProtectedRegion region : this.getRegionContainer().get(world)
                .getApplicableRegions(new ProtectedCuboidRegion("UnloadChunkFlagTester",
                        BlockVector3.at(chunk.getX() * 16, 0, chunk.getZ() * 16),
                        BlockVector3.at(chunk.getX() * 16 + 15, 256, chunk.getZ() * 16 + 15)))) {
            if (region.getFlag(Flags.CHUNK_UNLOAD) == State.DENY) {
                if (WorldGuardSevenCommunicator.supportsForceLoad) {
